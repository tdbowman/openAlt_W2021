...
def stackExchangeIngest(uniqueEvent, cursor, connection):
    ...
    # Author: Salsabil Bakth (lines 100-107)
    ...
    if(len(t_obj_id) < 100):
        ...
        if (type(row) == dict):
            ...
            firstEvent = row['firststackExchangeEvent']
            lastEvent = row['laststackExchangeEvent']
            if row['totalEvents'] is not None:
                totalEvents = row['totalEvents']
            if row['totalstackExchangeEvents'] is not None:
                totalstackExchangeEvents = row['totalstackExchangeEvents']
        elif (type(row) == tuple):
            ...
            if row[3] is not None:
                totalstackExchangeEvents = row[3]
    ...
    # If t_dateTime is less than firstEvent or if firstEvent is NULL, update firststackExchangeEvent with t_dateTime in the same row in the main table.
    ...
    # If t_dateTime is greater than lastEvent or if lastEvent is NULL, update laststackExchangeEvent with t_dateTime in the same row in the main table.
