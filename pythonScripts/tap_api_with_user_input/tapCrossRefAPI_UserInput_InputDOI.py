...
def main():
    ...
    fetchData(fetchURL, email, source, rows, objectID)


# Assemble the query for CrossRef, sleep and retry if the server is busy
def fetchData(fetchURL, email, source, rows, objectID):

    query = "curl " + "\"" + fetchURL + email + source + rows + objectID + "\"" + " > all-events.json" 
